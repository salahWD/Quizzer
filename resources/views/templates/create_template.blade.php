...
@section("content")
  ...
  <div class="container">
    <form class="my-4 py-4 needs-validation" method="POST" action="{{ route('store_template') }}">
      {{ csrf_field() }}
      ...
      <div class="mw-650">
        ...
        <div class="mb-4">
          ...
          <div class="d-none">
            <input class="form-check-input input-type" id="scoring" type="radio" name="type" value="1">
            <input class="form-check-input input-type" id="outcome" type="radio" name="type" value="2">
          </div>
          <div class="row">
            <div class="col-md-6">
        ...
        <div class="mb-4">
          ...
          <textarea
              required
              type="text"
              name="en_template_desc"
              id="templateDesc"
              class="form-control
              @if (count($errors) > 0)
                {{ count($errors->get("en_template_desc")) > 0 ? "is-invalid": "is-valid";}}
              @endif
              "
              value="{{ old("en_template_desc") ?? "" }}"
              placeholder="{{ __("Template Description (line or tow)") }}"></textarea>
          @if (count($errors->get("en_template_desc")) > 0)
            <div class="invalid-feedback">
              @foreach ($errors->get("en_template_desc") as $err)
                {{ $err }}
              @endforeach
            </div>
          @endif
